...
///////////////////////////////////////////////////////////////////////////////////////////////////
std::vector<PossiblePlate> detectCharsInPlates(std::vector<PossiblePlate> &vectorOfPossiblePlates, int &type) {
    ...
    for (auto &possiblePlate : vectorOfPossiblePlates)  // for each possible plate, this is a big for loop that takes up most of the function
    {
        ...
        preprocess(possiblePlate.imgPlate, possiblePlate.imgGrayscale, possiblePlate.imgThresh, type);        // preprocess to get grayscale and threshold images
        ...
        {
            if ( (vectorOfVectorsOfMatchingCharsInPlate[i].size() > intLenOfLongestVectorOfChars) && (vectorOfVectorsOfMatchingCharsInPlate[i].size() < 8)) {
        ...
        //MODIFICATION TO THE ORIGINAL CODE, SEARCH PLATES WITH 5-7 CHARACTERS AND VERIFY THEY HAVE A NONZERO CHARACTER VALUE TO VALIDATE THEM
        //THIS IN CASE THERE ARE MORE THAN ONE POSSIBLE PLATE WITH THE ACTUAL NUMBER OF CHARACTERS
        /*std::vector<std::vector<PossibleChar> > vectorOfVectorsOfPossibleRealPlates;

        for (unsigned int i = 0; i < vectorOfVectorsOfMatchingCharsInPlate.size(); i++)
        {
            if ( (vectorOfVectorsOfMatchingCharsInPlate[i].size() > 4) && (vectorOfVectorsOfMatchingCharsInPlate[i].size() < 8)) {
                vectorOfVectorsOfPossibleRealPlates.push_back(vectorOfVectorsOfMatchingCharsInPlate[i]);
            }
        }
        //TO SELECT THE MOST PROBABLE REAL PLATE IT MUST CONTAIN A NON ZERO NUMERICAL VALUE
        for(unsigned int i = 0; i < vectorOfVectorsOfPossibleRealPlates.size(); i++)
        {
            for (unsigned int idx = 0; i < vectorOfVectorsOfPossibleRealPlates[i].size() ; i++)
            {
                //UNDEFINED SINCE A BETTE SOLUTION COULD BE IMPLEMENTED FOR THE SPECIFIC "JALISCO" CASE
            }
        }
        std::vector<PossibleChar> longestVectorOfMatchingCharsInPlate = vectorOfVectorsOfMatchingCharsInPlate[intIndexOfLongestVectorOfChars];*/
